...
if($isLoggedIn) {
	...
	if($userDetails) {
		...
		if(isset($_SESSION["userType"]) && $_SESSION["userType"] == 1) {
			...
			// Check if user has assigned programme
			try {
				$progQuery = $con->prepare("
					...
					LIMIT 1
				");
				$progQuery->execute([$userId]);
				if($progQuery->rowCount() > 0) {
					$progRow = $progQuery->fetch(PDO::FETCH_ASSOC);
					$dashboardLink = 'main.php?ci=' . htmlspecialchars($progRow['programme_id']);
				}
			} catch(Exception $e) {
