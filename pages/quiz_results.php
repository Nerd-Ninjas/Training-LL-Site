...
// Fetch quiz details
$quizDetails = $account->getQuizInfo($quiz_id) ?: $account->getExamInfo($quiz_id);
...
// Fetch correct answers and statistics
$sql = "SELECT q.id, q.correct_option, q.explanation, 
        COALESCE(COUNT(ua.selected_option), 0) AS total_attempts, 
        COALESCE(SUM(CASE WHEN ua.selected_option = q.correct_option THEN 1 ELSE 0 END), 0) AS correct_count
        FROM questions q 
...
foreach ($user_answers as $question_id => $selected_option) {
    if (isset($correct_answers[$question_id]) && $selected_option == $correct_answers[$question_id]['option']) {
        $score++;
    }
}

$total_questions = count($questions);
$correct_percentage = $total_questions > 0 ? ($score / $total_questions) * 100 : 0;
$pass_percentage = isset($quizDetails['passingScore']) ? $quizDetails['passingScore'] : 50;
$pass_fail = $correct_percentage >= $pass_percentage ? "Passed" : "Failed";

// Insert into quiz_results
$sql = "INSERT INTO quiz_results (username, quiz_id, attempt_id, score, total_questions, percentage, status, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
$stmt = $conn->prepare($sql);
$status = $pass_fail; // Define $status before use
$stmt->bind_param("sssiiis", $user_id, $quiz_id, $attempt_id, $score, $total_questions, $correct_percentage, $status);
if ($stmt->execute()) {
    echo "<script>console.log('Results saved successfully.');</script>";
} else {
    echo "<script>console.log('Error: " . addslashes($stmt->error) . "');</script>";
}
$stmt->close();
if($status=="Passed"){
$course_type='CC';
$course_id=$quizDetails['course_id'];
$sql = "UPDATE users_certificates 
            SET approved = 1 
            WHERE username = ? 
            AND course_id = ? 
            AND type = ?";
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sss", $user_id, $course_id, $course_type);
