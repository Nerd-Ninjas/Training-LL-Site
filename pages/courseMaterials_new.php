 <?php
	  ...
	  $id=$_GET['id'];
	  $pdfViewer = $account->getCourseMaterials($id);
  ...
  <body>
    <div class="page-header">
      <div>
        ...
        <h1 class="page-title">Course Materials - <?php echo $title;?></h1>
      </div>
 ...
 <!-- PAGE-HEADER END -->
	...
	if($id!='MSTEEO'){
	  if($pdfViewer){
		...
		$pageUrl = $protocol . $_SERVER['HTTP_HOST'] . strtok($_SERVER["REQUEST_URI"], '?');
...
?>
    <div class="row">
      <div class="card">
		...
		<script>
        const pdfUrl = "<?php echo $filepath; ?>"; // Change this to your actual PDF file
            ...
            pdfjsLib.getDocument(url).promise.then(pdf => {
                ...
                for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                    pdf.getPage(pageNum).then(page => {
                        const scale = 2; // Increase scale for high resolution
                        const viewport = page.getViewport({ scale });

                        const canvas = document.createElement("canvas");
                        const context = canvas.getContext("2d");

                        // Use devicePixelRatio for better clarity on high-DPI screens
                        const dpr = window.devicePixelRatio || 1;
                        const adjustedScale = scale * dpr;
                        const highResViewport = page.getViewport({ scale: adjustedScale });

                        // Set higher resolution canvas
                        canvas.width = highResViewport.width;
                        canvas.height = highResViewport.height;

                        // Reduce canvas size visually while maintaining high resolution
	...
	<div class="row">
      <div class="card">
        <div class="card-body">
		<p>You are requested to download the excel sheet attached. Upload this downloaded file to your One Drive before the start on the session.</p>
		<p style="padding:5px;font-weight:bold;">Handouts</p>
			<p><i class="fa fa-file-excel-o fa-lg"></i><span style="padding:10px;"> Excel Online for Session </span><a href="<?php echo $filepath; ?>" target="_blank"><i class="fa fa-download fa-lg"></i></a></p>
        </div>
      </div>
    </div>
